...
TEST_CASE("Delay: set result and callback") {
    ...
    s.setCallback([&callCount](trial<T> &&r) {
        ++callCount;
...
TEST_CASE("Delay: set callback and result") {
    ...
    unsigned callCount = 0;
    s.setCallback([&callCount](trial<int> &&r) {
        CHECK(*r == 42);
        ++callCount;
    });
    CHECK(callCount == 0);
...
TEST_CASE("Delay: set result with throwing constructor and then callback") {
    ...
    unsigned callCount = 0;
    s.setCallback([&callCount](trial<Thrower> &&r) {
        ++callCount;
        try {
            *r;
        } catch (int i) {
...
TEST_CASE("Delay: simplest forward") {
    ...
    CHECK(target.unique());
    int result = 0;
    target->setCallback([&result](trial<int> &&r) { result = *r; });
    CHECK(result == 42);
}
...
TEST_CASE("Delay: forward with abandoned source") {
    ...
    CHECK(target.unique());
    target->setCallback([](trial<int> &&) { FAIL("unexpected"); });
}
...
TEST_CASE("Delay: forward with connected target") {
    ...
    int result = 0;
    target2->setCallback([&result](trial<int> &&r) { result = *r; });
    CHECK(result == 42);
}
