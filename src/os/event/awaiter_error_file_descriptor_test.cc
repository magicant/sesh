...
TEST_CASE_METHOD(
    ...
    expect_result(
            a.expect(error_file_descriptor(4)),
    ...
    implementation() = [this](
        ...
        check_equal(error_fds, {4}, fd_bound, "error_fds");
...
TEST_CASE_METHOD(
    ...
    mutable_steady_clock_now() = start_time;
    expect_result(
            a.expect(error_file_descriptor(2), error_file_descriptor(0)),
            [this, start_time](trigger &&t) {
        CHECK(t.tag() == trigger::tag<error_file_descriptor>());
    ...
    implementation() = [this](
        ...
        check_empty(read_fds, fd_bound, "read_fds");
        check_empty(write_fds, fd_bound, "write_fds");
        check_equal(error_fds, {0, 2}, fd_bound, "error_fds");
        CHECK(timeout.count() < 0);
        CHECK(signal_mask == nullptr);
        mutable_steady_clock_now() += std::chrono::seconds(3);
        error_fds->reset(2);
...
TEST_CASE_METHOD(
    ...
    expect_result(
            a.expect(error_file_descriptor(1)),
            [this, start_time](trigger &&t) {
        CHECK(t.tag() == trigger::tag<error_file_descriptor>());
    ...
    implementation() = [this](
        ...
        check_equal(error_fds, {1, 3}, fd_bound, "error_fds 1");
        error_fds->reset(3);
        CHECK(timeout.count() < 0);
        CHECK(signal_mask == nullptr);
        ...
        implementation() = [this](
            ...
            check_equal(error_fds, {3}, fd_bound, "error_fds 2");
            CHECK(timeout.count() < 0);
            CHECK(signal_mask == nullptr);
...
TEST_CASE_METHOD(
    ...
    auto callback = [this, start_time](trigger &&t) {
        CHECK(t.tag() == trigger::tag<error_file_descriptor>());
        CHECK(t.value<error_file_descriptor>().value() == 7);
        mutable_steady_clock_now() += std::chrono::seconds(1);
    };
    expect_result(a.expect(error_file_descriptor(7)), callback);
    expect_result(a.expect(error_file_descriptor(7)), callback);
    ...
    implementation() = [this, &count](
        ...
        check_empty(read_fds, fd_bound, "read_fds");
        check_empty(write_fds, fd_bound, "write_fds");
        check_equal(error_fds, {7}, fd_bound, "error_fds");
        error_fds->reset(3);
        CHECK(timeout.count() < 0);
        CHECK(signal_mask == nullptr);
        return std::error_code();
